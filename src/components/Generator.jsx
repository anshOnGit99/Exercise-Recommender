...
export default function Generator(props) {
  ...
  return (
    <SectionWrapper id={'generate'} header={"Generate your workout"} title={['It\'s','Huge ','o\'clock']}>
      ...
      <Header index={"02"} title={"Lock on Targets"} description={"Select the muscles judged for annihilaton."}/>
      <div className='bg-slate-950 border border-solid border-blue-400 rounded-lg flex flex-col'>
        ...
        {showModal && (
          <div className='flex flex-col px-3 pb-3'>
            {(poison==='individual'? WORKOUTS[poison] :Object.keys(WORKOUTS[poison])).map((muscleGroup,muscelGroupindex)=>{
              return( 
                <button onClick={()=>{
                  updateMuscle(muscleGroup)
                }} className={'hover:text-blue-100 duration-200'+(muscles.includes(muscleGroup)? ' text-blue-400':' ')} key={muscelGroupindex}>
